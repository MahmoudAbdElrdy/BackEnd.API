...
namespace BackEnd.API.ModelBinding
{
    public class JsonWithFilesFormDataModelBinder : IModelBinder
    {
        ...
        public async Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            var model = Newtonsoft.Json.JsonConvert.DeserializeObject(rawValue, bindingContext.ModelType, _jsonOptions.Value.SerializerSettings);

            // Now, bind each of the IFormFile properties from the other form parts
            foreach (var property in bindingContext.ModelMetadata.Properties)
            {
                if (property.ModelType != typeof(Microsoft.AspNetCore.Http.IFormFile))
                    continue;
